...
const getIsRegistrationOpen = event => {
  if (event.registrationOpens) {
    const endDate = event.registrationCloses ? event.registrationCloses : new Date(8640000000000000)
...
const mapStateToProps = state => {
  const latestEvent = R.pathOr({}, eventPath.concat(["data", "currentEvent"]), state)
  ...
  const isFeedbackOpen = R.pathEq(eventPath.concat(["data", "feedbackOpen"]), true, state)
